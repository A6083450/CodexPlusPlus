...
pub fn prepare_dream_skin_activation(
    ...
    let stored_image = load_stored_dream_skin_theme(state_dir, &draft.config.id)
        .ok()
        .filter(|stored| Path::new(&stored.image_path) == source)
        .map(|stored| PathBuf::from(stored.image_path));
    let active_image = if let Some(stored_image) = stored_image {
        let metadata = std::fs::symlink_metadata(&stored_image)?;
        if !metadata.file_type().is_file() || metadata.file_type().is_symlink() {
            bail!("stored Dream Skin image is not a safe file");
        }
        if metadata.len() == 0 {
            bail!("stored Dream Skin image is empty");
        }
        if metadata.len() > crate::dream_skin::DREAM_SKIN_PREPARED_LIMIT {
            bail!("stored Dream Skin image exceeds 16 MiB");
        }
        let extension = stored_image
            .extension()
            .and_then(|value| value.to_str())
            .map(str::to_ascii_lowercase)
            .context("stored Dream Skin image has no extension")?;
        std::fs::create_dir_all(&managed_dir)?;
        let active_image = managed_dir.join(format!("current.{extension}"));
        crate::settings::atomic_write(&active_image, &std::fs::read(&stored_image)?)?;
        active_image
    } else {
