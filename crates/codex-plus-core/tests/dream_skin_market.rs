...
async fn installs_verified_market_theme_into_local_library() {
    ...
    let theme_dir = state.path().join("dream-skin/themes/market-demo");
    assert!(theme_dir.join("theme.json").is_file());
    let stored_image = Path::new(&stored.image_path);
    assert!(stored_image.is_file());
    assert!(stored_image.starts_with(&theme_dir));
    assert_eq!(
        stored_image.file_name().and_then(|name| name.to_str()),
