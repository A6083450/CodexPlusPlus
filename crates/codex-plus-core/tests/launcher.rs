...
/// 普通 helper 端口在非 macOS 上已经挑过空闲的了，占用时应立即失败；
/// macOS 重启则允许旧 launcher 用六秒释放残留 socket（#1990）。
#[tokio::test]
async fn a_busy_floating_helper_port_only_retries_during_macos_restart() {
    let temp = tempfile::tempdir().unwrap();
    let app_dir = temp.path().join("Codex.app");
    std::fs::create_dir_all(&app_dir).unwrap();
    let status_store = StatusStore::new(temp.path().join("latest-status.json"));
    ...
    let expected_attempts = if cfg!(target_os = "macos") { 31 } else { 1 };
    assert_eq!(
        events
            .lock()
            .unwrap()
