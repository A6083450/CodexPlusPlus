...
#[test]
fn gpt6_metadata_exposes_max_ultra_in_catalog_and_ui() {
    use codex_plus_core::model_suffix::requires_bundled_metadata_catalog;

    for (slug, display) in [("gpt-6-astra", "GPT-6-Astra"), ("gpt-6-sol", "GPT-6-Sol"), ("gpt-6-luna", "GPT-6-Luna")] {
        assert!(requires_bundled_metadata_catalog(slug), "{slug}");
        let unknown = format!("{slug}-custom");
        assert!(!requires_bundled_metadata_catalog(&unknown));
        assert!(model_ui_metadata(&unknown).is_none());
        let entries =
            collect_catalog_entries(slug, &HashMap::new(), &HashMap::new(), "");
        let catalog: serde_json::Value =
            serde_json::from_str(&build_model_catalog_json(&entries, None)).unwrap();
        let model = &catalog["models"][0];
        let metadata = model_ui_metadata(slug).unwrap();
