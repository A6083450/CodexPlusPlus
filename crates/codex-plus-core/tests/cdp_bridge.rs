...
fn deferred_runtime_scripts_are_only_scheduled_for_initial_bridge_install() {
    ...
    assert!(source.contains("if !deferred_runtime_scripts.is_empty()"));
    assert!(source.contains("bridge_generation_is_current(&deferred_generation)"));
    assert!(!source.contains("replacing_active_pump"));
}
...
#[test]
fn injection_script_recovers_generated_images_runtime() {
    let script = assets::injection_script(57321);
...
fn injection_script_keeps_session_action_buttons_in_pr_style() {
    ...
    assert!(script.contains("background: var(--codex-plus-bg-elevated);"));
    assert!(
...
const usageManager = {{ requestClient: nativeContextClient, getConversation: (id) => id === "thread-native-context" ? usageConversation : null, isConversationStreaming: () => usageStreaming, updateConversationState: (_, update) => update(usageConversation), addAnyConversationCallback(listener) {{ usageListeners.add(listener); return () => usageListeners.delete(listener); }}, async resumeConversation() {{ usageConversation.latestTokenUsageInfo = {{ ...usageConversation.latestTokenUsageInfo, modelContextWindow: 258400 }}; usageListeners.forEach(listener => listener("thread-native-context")); return {{ status: "ready" }}; }} }};
